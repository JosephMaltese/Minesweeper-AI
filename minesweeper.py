...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        # 3) add a new sentence to the AI's knowledge base
        #    based on the value of `cell` and `count`
        neighbours, newCount = self.find_neighbours(cell, count)

        if neighbours:
            newSentence = Sentence(neighbours, newCount)
            if newSentence not in self.knowledge:
                if newSentence.known_safes():
                    for element in newSentence.cells:
                        self.mark_safe(element)
                elif newSentence.known_mines():
                    for element in newSentence.cells:
        ...
        for sentence in self.knowledge:
            if len(sentence.known_mines()) != 0:
                knownMines = list(sentence.known_mines())
                for mine in knownMines:
                    self.mark_mine(mine)
            if len(sentence.known_safes()) != 0:
                knownSafes = list(sentence.known_safes())
        ...
        for sentence1 in self.knowledge:
            for sentence2 in self.knowledge:
                if sentence1 != sentence2 and sentence1.cells.issubset(sentence2.cells):
                    # If this point is reached, sentence1 is a subset of sentence2
        ...
        self.knowledge = [sentence for sentence in self.knowledge if sentence.count != 0]
